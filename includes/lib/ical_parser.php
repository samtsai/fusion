<?php
$day_array = array("0" => "U", "1" => "M", "2" => "T", "3" => "W", "4" => "R", "5" => "F", "6" => "S");
$day_short_names = array ("MO" => "M", "TU" => "T", "WE" => "W", "TH" => "R", "FR" => "F", "SA" => "S", "SU" => "U");
$next_day_short_names = array("MO" => "T", "TU" => "W", "WE" => "R", "TH" => "F", "FR" => "S", "SA" => "U", "SU" => "M");
/*
include_once('./date_functions.php');
include_once('./calendar_functions.php');
//include_once('./overlapping_events.php');
include_once('./timezones.php');
*/

//$cal_filelist = array(1 => "1.ics");
$parse_file = true;
...
foreach ($cal_filelist as $cal_key => $filename) {
	...
	if ($parse_file) {
		...
		while (!feof($ifile)) {
			...
			switch ($line) {
				case 'BEGIN:VEVENT' :
					//echo "start $start_unixtime, end $end_unixtime, duration $the_duration<br/>";
...
//					echo "start time $start_unixtime, end time $end_unixtime<br/>";
					...
					if ((isset ($allday_start) && $allday_start != '')) {
						...
						if (($start < $mArray_end) && ($start < $end)) {
							while (($start != $end) && ($start < $mArray_end)) {
								...
								), 'attendee' => serialize($attendee), 'calnumber' => $calnumber, 'calname' => $actual_calname, 'url' => $url, 'status' => $status, 'class' => $class);
								$start = strtotime('+1 day', $start);

							}
							//echo "NEW EVENT!";
							//$counter++;
							//print_r($master_array[($start_date2)]);
...
//								echo "start_time $start_tmp, end_time $end_unixtime<br/>";
								...
								$master_array[$start_date_tmp][$time_tmp][$uid] = array (
									...
									'location' => $location,
									'organizer' => serialize($organizer
								), 'attendee' => serialize($attendee), 'calnumber' => $calnumber, 'calname' => $actual_calname, 'url' => $url);
...
//							echo "start_time $start_time, end_time $end_time<br/>";
							if ($bleed_check == '-1') {
								$display_end_tmp = $end_time;
								$end_time_tmp1 = '2400';
							}
														...
														if (isset ($allday_start) && $allday_start != '') {
															...
															while ($start_time2 < $end_time2) {
																...
																$master_array[($start_date2)][('-1')][$uid] = array (
																	...
																	'location' => $location,
																	'organizer' => serialize($organizer
																), 'attendee' => serialize($attendee), 'calnumber' => $calnumber, 'calname' => $actual_calname, 'url' => $url, 'status' => $status, 'class' => $class, 'recur' => $recur);
																...
																while ($start_tmp < $end_unixtime_tmp) {
																	...
																	if ($abs_until > $until_check) {
																		$master_array[$start_date_tmp][$time_tmp][$uid] = array (
																			...
																			'location' => $location,
																			'organizer' => serialize($organizer
																		), 'attendee' => serialize($attendee), 'calnumber' => $calnumber, 'calname' => $actual_calname, 'url' => $url, 'recur' => $recur);
																		if (isset ($display_end_tmp)) {
																			$master_array[$start_date_tmp][$time_tmp][$uid]['display_end'] = $display_end_tmp;
																		}
					...
					//TODO//
					$ical_event = $master_array[($start_date)][($hour . $minute)][$uid];
					if(!empty($ical_event["event_text"]))
					{
						/*
						echo "<pre>";
						print_r($ical_event);
						...
						echo "<br/><br/>";
						*/
						$eventName = convertiCalText($ical_event["event_text"]);
						$eventDescription = convertiCalText($ical_event["description"]);
						$eventDescription .= "  Location: " . convertiCalText($ical_event["location"]);
						
						if($ical_event["start_unixtime"] == $ical_event["end_unixtime"])
						{
							$ical_event["end_unixtime"] += 300;
						}
						
						//echo "Name: $eventName, <br/>Desc: $eventDescription<br/>";
						//echo "Start Time: ".$ical_event["start_unixtime"];
						//echo " End Time: ".$ical_event["end_unixtime"]."<br/>";

						//if event wraps midnight
						if(date("w",$ical_event["start_unixtime"]) !== date("w", $ical_event["end_unixtime"]))
						{
							$date_data = getdate($start_unixtime);
							$set_end_unixtime = mktime(23,55,0,$date_data["mon"],$date_data["mday"],$date_data["year"]);
							$set_start_unixtime = mktime(0,5,0,$date_data["mon"],$date_data["mday"]+1,$date_data["year"]);
							//add the second half event
							$event = new Event();
							$event->setEventName($eventName);
							$event->setPersonId($_SESSION["person_id"]);
							$event->setEventType(3);
							$event->setEventDescription($eventDescription);
							$event->setStartDatetime($set_start_unixtime);
							$event->setEndDatetime($ical_event["end_unixtime"]);

							if (isset ($ical_event["recur"]) && $ical_event["recur"]["FREQ"] != "yearly" && $ical_event["recur"]["FREQ"] != "monthly") {
			
								$days_array = array ();
								if(isset($ical_event["recur"]["BYDAY"]) && !empty($ical_event["recur"]["BYDAY"]))
								{
									//echo "setting recurrence byday <br/>";
									foreach ($ical_event["recur"]["BYDAY"] as $day) {
										
											$days_array[] = $next_day_short_names[$day];
									}
									$day_str = trim(implode("|", $days_array));
								}
								else
								{
									//echo "no byday recurrence specified, taking start day as default <br/>";
									$day_str = $day_array[date("w",$ical_event["start_unixtime"])];
															
								}
		
								
								$event->setRecurrenceStart($ical_event["start_unixtime"]);
								
								if(isset($ical_event["recur"]["UNTIL"]) && !empty($ical_event["recur"]["UNTIL"]))
								{
									$event->setRecurrenceEnd($ical_event["recur"]["UNTIL"]);
								}
								else
								{
									$event->setRecurrenceEnd(FOREVER);
								}
								
								$event->setEventDescription($eventDescription);
								if(isset($ical_event["recur"]["INTERVAL"]) && !empty($ical_event["recur"]["INTERVAL"]))
									$event->setRecurrenceWeeks($ical_event["recur"]["INTERVAL"]);
								else
									$event->setRecurrenceWeeks(1);	
									
								//echo "setting recurrence weeks to " . $ical_event["recur"]["INTERVAL"]."<br/>";
								$event->setRecurrenceDays($day_str);
								//echo "setting recurrence days to $day_str<br/>";
								
							}
	/*						else
							{
								echo "no recurrence data that we can handle<br/>";
							}
							echo "<br/>";
							...
							{
								$iCalErrors = true;
								$errors .= "<div class='error'>".$event->getConflictsMessage();
								$errors .= "</div><br/>";
							}
							else
							{
								echo "Event '$eventName' Successfully added!<br/>";
							}
						...
						{
							$set_end_unixtime = $ical_event["end_unixtime"];							
						}
						...
						$event->setEventType(3);
						$event->setEventDescription($eventDescription);
						$event->setStartDatetime($ical_event["start_unixtime"]);
						$event->setEndDatetime($set_end_unixtime);
						...
						if (isset ($ical_event["recur"]) && $ical_event["recur"]["FREQ"] != "yearly" && $ical_event["recur"]["FREQ"] != "monthly") {
							...
							if(isset($ical_event["recur"]["BYDAY"]) && !empty($ical_event["recur"]["BYDAY"]))
							{
								//echo "setting recurrence byday <br/>";
								foreach ($ical_event["recur"]["BYDAY"] as $day) {
										$days_array[] = $day_short_names[$day];
								}
								$day_str = trim(implode("|", $days_array));
							}
							else
							...
							if(isset($ical_event["recur"]["UNTIL"]) && !empty($ical_event["recur"]["UNTIL"]))
							{
								//echo "setting recurrence until to " .$ical_event["recur"]["UNTIL"];
								$event->setRecurrenceEnd($ical_event["recur"]["UNTIL"]);
							}
							else
							{
								//echo "setting recurrence until to FOREVER<br/>";
								//echo "<pre>";
								//print_r($ical_event);
								//echo "</pre>";
								$event->setRecurrenceEnd(FOREVER);
						...
						switch ($property) {
							...
							*/
							...
							case 'ATTENDEE' :
								...
								$attendee[] = array (
									'name' => stripslashes($field
								), 'email' => stripslashes($data));
								break;
							case 'ORGANIZER' :
...
$template_started = microtime(true);
...
echo "<br/><br/><br/><br/><br/>";
echo "recurrence array::";
print_r($rrule_array);

echo "<br/><br/><br/><br/><br/>";
echo "recurrence-del array::";
...
//this currently handles only 2digit hex values.  
//meaning that there is almost certianly input that breaks this
function convertiCalText($text)
{
	$spaced = str_replace("+", " ",$text);
	for($i = 0; $i < strlen($spaced); $i++)
	{
		if ($spaced{$i} == "%")
		{
			$find = substr($spaced,$i,3);
			$replace = hex2ascii(substr($spaced,$i+1,2));
			$spaced = str_replace($find,$replace,$spaced);
		}
	}
	return unhtmlspecialchars($spaced);
}
